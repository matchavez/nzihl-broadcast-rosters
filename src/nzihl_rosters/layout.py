"""Render the single-page roster PDF for any two NZIHL teams.
...
# House font: Inter, with tabular figures ('tnum') and full numerical
# disambiguation ('ss02' — dotted/slashed zero, serif capital I, tailed
# lowercase l) frozen into the font's cmap as the *default* glyphs. reportlab's
# TTFont embedding draws straight from cmap with no OpenType shaping engine, so
# activating those features at draw time isn't possible — baking them into the
# font itself is. registerFont embeds the outline data directly in the PDF.
...
def build_roster_pdf(
    *,
    ...
    def _draw_header_badge_and_name(x: float, team: Team, title_color, band_top: float, band_h: float):
        """Draw the team name centred, with its logo on a white rounded chip
        immediately to the left, the pair centred together in the band."""
        ...
        logo_box = HEADER_FS * 1.5      # drawn logo size (pt) ~ a touch over cap height
        pad      = 1.6                  # white border around the logo
        chip     = logo_box + 2*pad
        gap      = 2.2*mm
        group_w  = chip + gap + text_w
        start_x  = x + col_w/2 - group_w/2

        # white rounded chip
        chip_x = start_x
        chip_y = band_mid - chip/2
        c.setFillColor(white); c.setStrokeColor(white); c.setLineWidth(0)
        c.roundRect(chip_x, chip_y, chip, chip, chip*0.22, fill=1, stroke=0)

        # logo, centred in the chip, aspect preserved, alpha respected
        c.drawImage(str(logo_path), chip_x + pad, chip_y + pad,
                    logo_box, logo_box, mask='auto', preserveAspectRatio=True)

        # team name
        c.setFillColor(title_color); c.setFont(FONT_BOLD, HEADER_FS)
        c.drawString(start_x + chip + gap, baseline, text)
    ...
    def draw_team(x: float, team: Team, skaters: list[SkaterRow], played_goalies: list[GoalieRow], coaches: list[CoachRow]):
        ...
        def draw_row(r: SkaterRow, ry: float, row_h: float, dim: bool):
            ...
            # treatment as the name) both live here now instead of two
            ...
            # Same weight as G/A, just 1pt smaller (Mat, 2026-07-02 — Thin
            # weight anti-aliased into a washed-out grey at this size, so back
            # to Regular, sized down instead of lightened).
            if r.plus_minus:
                pm = r.plus_minus
