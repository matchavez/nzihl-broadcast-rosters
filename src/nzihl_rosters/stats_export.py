...
from matchavez/nzihl-season-data's committed warehouse (nzihl.json's
`derived.player_game_logs`) instead of stats_1team.cfm's own G/A/PTS
columns, verified byte-for-byte against every skater on all 5 NZIHL teams.
stats_1team.cfm is still scraped for every OTHER skater field (jersey,
position, flag, GP, PIM) and for every goalie/coach field -- this repo
just swaps the SOURCE of two numbers, the output shape here is unchanged.
See `_warehouse_goals_assists` for the matching rule and why PIM/goalies
were deliberately NOT migrated (verification found real, unexplained
mismatches for both -- see this repo's memory.md).
"""
...
_SEASON_DATA_URL = "https://raw.githubusercontent.com/matchavez/nzihl-season-data/main/nzihl.json"
...
def _warehouse_goals_assists(raw_name: str, game_logs: dict) -> tuple[int, int] | None:
    ...
    (Niskakoski)"), so normalizing the UN-overridden raw name is what
    actually matches its key -- normalizing the override-corrected
    first/last (e.g. "Reagyn Shattock") would silently miss.
