...
def fetch_personnel_html(team_id: int, client_id: int = 7131, league_id: int = 35499) -> str:
...
def _split_first_last(full_name: str) -> tuple[str, str]:
    ...
    # Two-word surnames like "Hayward Jones" -- checked against the shared
    # allowlist in overrides.py (referenced via the module object, not a bare
    # `from .overrides import MULTI_WORD_SURNAMES`, so a later
    # load_remote_overrides() rebind is actually picked up here); default to
    # splitting at the last space.
