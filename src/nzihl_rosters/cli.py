...
Behaviour:
    1. Fetch the NZIHL schedule.
    2. Keep games whose start time falls inside the window (default 4 days).
    3. Expand to the rest of each game's series (same two teams, <=3 days
       apart) even if part of it falls outside the window — a weekend series
       is never split across runs.
    4. Group consecutive games between the same two teams into a series.
    5. For each series, scrape both team rosters and render a single PDF.
    6. Filename: <YYYY-MM-DD>_<AWAY>_at_<HOME>.pdf
"""
...
def _round_label(start: datetime) -> str:
    """Crude round label. NZIHL doesn't expose this in the schedule HTML, so we
    derive it from the calendar week of the season. Refine as needed."""
    # Season opens 8 May 2026 -> week 1; round 03 begins 22 May.
    # Round number = ((days_since_opener) // 7) + 1
...
def _date_label(series: list[Game]) -> str:
    """Combine the game dates+times for the footer line."""
    parts = []
    for g in series:
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Build NZIHL roster PDFs for upcoming games.")
    ...
    # entries that have aged out — otherwise a stale card (e.g. a past game,
    # with nothing yet scheduled to replace it) sits on the portal forever,
    # since no later run would ever touch the file again.
